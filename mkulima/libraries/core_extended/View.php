...
class View
{
    ...
    public function discussion_answer($question_id){
        return $this->controller->discussion_model->main_comments($question_id);
    }

    public function show_answer_comment($answer_id){
        return $this->controller->discussion_model->show_answer_comment($answer_id);
    }
   
    public function categories(){
        return $this->controller->publish_model->categories();
    }

    public function view_category($username){
        echo $this->controller->accounts_model->check_category($username);
    }

    public function show_category($cat_id){
        echo $this->controller->accounts_model->find_category($cat_id);
    }

    public function gravator($username){
            $gravator=$this->controller->discussion_model->gravator($username);
            if($gravator){
    ...
    public function author_details($username){
        return $this->controller->publish_model->author_details($username);
    }

    public function fullname($username){
        foreach($this->controller->accounts_model->profile($username) as $row){
            echo "{$row->first_name} {$row->last_name}";
        }

    }

    public function response_counts(){
        return $this->controller->discussion_model->counts('discussion_answers');
    }

    public function question_counts(){
        return $this->controller->discussion_model->counts('user_questions');
    }
